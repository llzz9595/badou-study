	    ...
	    if(/\./g.test(str)){
	        if(formatFloat!=null){
	            var tempFloat = Math.round(parseFloat('0.'+str.split('.')[1])*Math.pow(10,formatFloat.length))/Math.pow(10,formatFloat.length);
	             if(number<0) {
	            	strInt = (Math.ceil(number)+Math.floor(tempFloat)).toString();
	            	if (number > -1) {  // 如果要格式化的数字大于-1，需要增加一个负数的符号，否则格式化出来的数值会变成整数
	            		strInt = "-" + strInt;
	            	}
	            } else {
...
var DialogUtil = {
	...
	**/
	...
	modalDialog:function(url,dialogArguments,dialogConfig ) {	
		return window.showModalDialog(url,dialogArguments,"dialogWidth:900px; dialogHeight:560px");
...
var SelectUtil = {
	...
	save:function(selectDOM,defaultText) {
		...
		if(optionsLength > 0) {
			...
			outputDatas = selectDOM.options[0].datas || '';
			for(var i=1;i<selectDOM.options.length;i++) {
				...
				outputDatas = outputDatas + "," + (selectDOM.options[i].datas || '');
			}
			// 向父窗口传递参数
			if(parent.window!=null){
				parent.window.returnValue = {outputText:outputText,outputValue:outputValue,outputDatas:outputDatas};
			}else{
				window.returnValue = {outputText:outputText,outputValue:outputValue,outputDatas:outputDatas};
			}
			
		} else {
			// 向父窗口传递参数
			if(parent.window!=null){
				parent.window.returnValue = {outputText:defaultText,outputValue:'',outputDatas:outputDatas};
			}else{
				window.returnValue = {outputText:defaultText,outputValue:'',outputDatas:outputDatas};
			}
	...
	addOptions:function(selectDOM,text,value,datas) {
		// 添加option
		// 相同option标志位
		...
		if(!hasOptions) {
			...
			if(datas) newOption.datas = datas;  
	...
	removeOption:function (selectDOM) {
		...
		if(length > 0){
 			for(var i=length-1;i>=0;i--){
   				if(selectDOM.options[i].selected)selectDOM.options.remove(i);
  			}
		}
	},
	...
	urlSerialize:function(cbName) {
		// 把name属性等cbName的dom对象的值序列化成以下的字符串
		// cbName=value1&cbName=value2&cbName=value3
		var cbs = Util.$_EN(cbName);
		var values = '';
		if(cbs!=null) {
			for(i=0;i<cbs.length;i++) {
...
function getAddressList(type,multi,manage,targetTextId,targetValueId,onReturn,orgids){
	...
	var addressUrl=context+"/web/system/dlg/org_select_dlgPage.jsp?type="+type+"&manage="+manage+"&multi="+multi+"&oids="+orgids;
	...
	enableHighlightTr:function(t) {
		...
		if(table!=null) {
			...
			if (table.addEventListener) {
				table.addEventListener("onmouseover", TableUtil.highlightTr, false); //firefox
				table.addEventListener("onmouseout", TableUtil.highlightTr, false); //firefox
				
			} else if (table.attachEvent) {//IE模式下
			...
			if(cbs !=null && cbs.length>0) {
				for(var i=0;i<cbs.length;i++) {
					...
					if (cbs[i].addEventListener) {
						cbs[i].addEventListener("onclick", TableUtil.hightlightSelectedTR, false); //firefox
		
						
					} else if (cbs[i].attachEvent) {//IE模式下
