...
function ev_reload(){
	var fm = $_E('form1');
	fm.action = "${ROOT}/editFlowGradeNodeScaleSet.do1?flowId=${flowId}";
	fm.submit();
}
...
<body class="LN_body">
<form name="form1" method="post" action="${ROOT }/saveFlowGradeNodeScaleSet.do1" id="form1">	
   ...
   <br/>
    ...
    <tr>
     ...
     <input type="hidden" name="scaleflowId" value="${o.id}"/>
     	...
     	<input type="text" name="flowGradeScale" class="LN_Input30" value="${o.gradeScale}" check="notBlank;isInt();setNumber(0,100)" required='true' showName='权重值'/></td></tr>
    <c:forEach var="o1" items="${mainList}" varStatus="idx">
      ...
      <tr>
        ...
        <input type="text" name="gradeScale" class="LN_Input30" value="${o1.gradeScale}" check="notBlank;isInt();setNumber(0,100);" required='true' showName='权重值'/>
		</td>
      </tr></c:if>
      </c:forEach>
     </c:forEach>
