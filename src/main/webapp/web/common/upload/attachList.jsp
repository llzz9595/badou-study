<%@ page language="java"
	import="java.util.*,com.shengdai.kpi.attach.model.*,com.shengdai.kpi.system.SpringHelper,com.shengdai.kpi.common.attach.service.IAttachService"
	pageEncoding="UTF-8"%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script>
		...
		function deleteAttach(id){	
			if (confirm("您确认要删除记录？")) {
				var url = "${ROOT}/deleteAttachFile.do1?id="+id;
		...
		*/
		function attachCopy() {
			if(SelectUtil.checkedCount("selectedrow") == 0) {
				...
				var url = context+"/copyAttachAjax.do1?idStr=" + idStr + "&randomKey=" + Math.random();
				Util.callScript(url);
			}			
		}
		...
		function attachPaste(resourceId) {
			if(StringUtils.isEmpty(resourceId)) {
				...
				var url = context + "/pasteAttachAjax.do1?resourceId=" + resourceId + "&randomKey=" + Math.random();
				Util.callScript(url);
			}		
		}
...
<%--						onmouseout='this.style.backgroundColor="#ffffff"'--%>
						...
						<c:choose>
							...
							<a href='${ROOT}/downloadFile.do1?id=${attachId}'>
								下载 
