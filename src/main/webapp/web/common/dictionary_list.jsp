...
function ev_openForm(id) {
	var winHeight = 300;
	var winWidth = 750;
	window.open('${ROOT}/viewDictionary.do1?did=' + id, '', 'menubar=no,toolbar=no,location=no,status=no,resizable=no,height='+winHeight+', width='+winWidth+',top='+(screen.height-winHeight)/2+',left='+(screen.width-winWidth)/2+',scrollbars=yes');
}
...
function ev_del() {
	...
	if (confirm("确定删除选定的字典？")) {
		...
		fm.action = "${ROOT}/batchDeleteDictionary.do1";
		fm.submit();
	}
}
...
function orderDic() {
	...
	var fm = document.getElementById("form1");
	fm.action = "${ROOT}/changeDictionaryOrder.do1";
	if (CheckForm(fm)) {
		fm.submit();
	}
}
		...
		<form name="form1" method="post"  id="form1" style="margin-top: 0px" onsubmit="return CheckForm(this)">
	        	...
	        	<tbody>
	        		...
	        		<tr>  
						<td class="list-caozuo">
							<a href="${ROOT}/viewDictionary.do1">新增</a>|
			...
			<table class="list-box" cellspacing="0" border="0">
				...
				<tr class="list-color${idx.index%2==0?'1':'2'}">
					...
					<td class="list-text1"><a href="${ROOT}/viewDictionary.do1?did=${o.id}" title="点击进行修改">${o.code}</a></td>
					<td class="list-text1"><a href="${ROOT}/viewDictionary.do1?did=${o.id}" title="点击进行修改">${o.name}</a></td>
