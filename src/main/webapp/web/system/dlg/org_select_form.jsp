		...
		<script>		
			...
			*/
			function addSelectedOrgs(to) {
				...
				//firefox不支持docment.frames  所以统一修改成window.frames
				var from=window.frames["fbody"].document.getElementById("orgOptions");
				...
				if(from.options.length > 0) {
					for(var i=0;i<from.options.length;i++) {
						...
						if(from.options[i].selected==true){
							...
							SelectUtil.addOptions(to,from.options[i].text,from.options[i].value,from.options[i].datas);
						}
					}
				}
			...
			*/
			function searchOrg() {
				...
				window.frames("fbody").document.getElementById("keyWord").value=keyWord;		//关键字
				window.frames("fbody").document.getElementById("type").value="<%=type%>";		//查询类型
				window.frames("fbody").document.getElementById("manage").value="<%=manage%>";		//查询权限

				window.frames("fbody").optionAction.action="${ROOT}/addressSearch.do1";
				...
				<%
					...
					%>
					var tmpUrl="${ROOT}/addressSearch.do1?type=<%=type%>&manage=<%=manage%>&keyWord="+keyWord;
					
					<%
					if(myGroup){
						//查询范围
						out.print("window.frames('fbody').document.getElementById('scope').value=tmpScope");
					}
					...
					window.frames("fbody").document.getElementById("searchFlag").value=1;		//查询标记
					window.frames("fbody").optionAction.submit();		//查询
				<% 
				...
				%>		
						//window.frames("fbody").location.href = '${ROOT}/addressSearch.do1';
					return ;
				<% 	}
				%>
				}
			...
			*/
			...
			function callback(){
				...
				parent.ftree.document.getElementById("mapFrame13").src="<%=request.getContextPath()%>/treePersonData.do1?jsFunc=list_folder&listType=addr&"+Math.random();
			    return;
			}
			...
			function favorite()
			{
			    ...
			    if(retValue!=""&&retValue.length>2)
			    {
			      ...
			      // {
			          //parent.ftree.document.getElementById("mapFrame13").src="<%=request.getContextPath()%>/treePersonFolder.do1?jsFunc=list_folder&listType=addr&"+Math.random();
			       //alert("mapFrame7:"+parent.ftree.document.getElementById("mapFrame7"));
			     //  }
			        
			       //alert(retArray.length);
			       var url = "<%=request.getContextPath()%>/createFavorite.do1?favoriteInfo="+encodeURI(outputValue)+"&parentId="+retArray[1];
			...
			function setRemarkAsync(){
				var options = selectedOrg.options;
				var retValue="";
		
				for(var i=0;i<selectedOrg.options.length;i++){
					if(selectedOrg.options[i].selected){
						if(retValue!=""){
							retValue+=","+selectedOrg.options[i].value;
						}else{
							retValue+=selectedOrg.options[i].value;
						}						
					}
				}
				var url = "${ROOT}/infoInAddressBook.do1?orgInfo="+retValue;
				LoadAjaxContentSyn(url,function() {
					$_E('addressRemark').innerHTML = Trim(this.request.responseText);
				}); 
			}
		</script>
					...
					<table width="93%" border="0" align="center" cellpadding="0"
						...
						<tr>
							<td colspan="2" class="color-explain" >
								...
								%>
								颜色说明：
								<span class="fontcolor0">部门</span> |
								<span class="fontcolor20">员工</span> |
								<span class="fontcolor40">角色</span> |
								<%
								}
								%>
							</td>
						...
						<tr>
								...
								<table width="100%" border="0" align="center" cellpadding="0"
									...
									<tr>
										<td class="address-text">
											<iframe id="fbody" src="${ROOT}/orgOptions.do1" scrolling="no"
							...
							<td align="center">
								<table width="250" border="0" cellspacing="0" cellpadding="0">
									...
									<tr>
										<td class="address-text">
											<select name="selStyle" onchange="setRemarkAsync();"
