	...
	<title>菜单信息</title>
		...
		<script type="text/javascript" src="">
			...
			function select_org_by_type(id,name) {
				...
				if(id != 'root') {
					//targetTypeID = id;
					parent.parent.fbody.fbody.location.href='${ROOT}/orgOptions.do1?id='+id+"&type=<%=type%>";
					
				}else{
					<%
					if(OrgHelper.getCurrentUserAddressScope()==0){			//当前用户是查询全省用户的时候，点中国电信广东分公司才在待选列表中显示内容
					%>
						parent.parent.fbody.fbody.location.href="${ROOT}/orgOptions.do1?id=<%=Globals.ROOT_ID%>&type=<%=type%>";
					<%}
					%>
