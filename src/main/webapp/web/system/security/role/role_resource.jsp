...
<%
	...
	if(role.getParent() != null && role.getParent().isRoot()){
		IRoleService roleService = (IRoleService)SpringHelper.getBean("roleService");
		Role sa = roleService.findById("1");
	...
	<!--
		...
		*/
		...
		function grantResource(){
			...
			form.action = "${ROOT}/grantResource.do1";
			form.submit();
		}
		
		//浏览角色下的人员
		function viewRoleUser(){
			window.location.href = "${ROOT}/viewRoleUser.do1?id=<%=role.getId()%>";
		}
		
		//添加角色
		function ev_add(id){
			var url = "${ROOT}/createRole.do1?id="+id;
		...
		function ev_edit(id){
			...
			var url = "${ROOT}/editRole.do1?id="+id;
			window.location.href = url;
		}
		...
		function ev_delete(id){
			...
			if(confirm("确定要删除角色吗?")){
				var form = document.getElementById("form");
				form.action = "${ROOT}/deleteRole.do1";
				form.submit();
			}
		}
		...
		*/
		function ev_setContorlDomain(resourceId,roleId){
			var url = "${ROOT}/listContorlDomain.do1?resourceId="+resourceId+"&roleId="+roleId;
...
<table width="95%" border="0" border="0" cellpadding="0" cellspacing="0">
<%
 ...
 %>
	<tr>
		<td style="border-bottom:#CCCCCC 1px solid;" align="left">
		<input  type="checkbox" id="sub<%=layer1Resource.getId() %>"  name="rid" value="<%=layer1Resource.getId() %>"
			onclick="selectAll(this);" class="checkboxInput" <%=resourceList.contains(layer1Resource.getId())?"checked":"" %>  />
 	...
 	%>
				<tr >	    	
 					<td>
						...
						<%=resourceList.contains(layer2Resource.getId())?"checked":"" %> />
