...
<html>
  <head>
	...
	<!--
		...
		var url = "${ROOT}/viewRoleResource.do1";
		//赋予员工一个系统角色
		function grantRoleUser(){
			...
			getAddressList('20','n','3','userText','userInfo');
			var userInfo = document.getElementById("userInfo").value;
			if(userInfo == ""||String(userInfo).length<=0){
				alert("没有选择任何数据");
				return;
			}
			document.form.action = "${ROOT}/grantRoleUser.do1";
		...
		function depriveRoleUser(){
			...
			if(confirm("确定要删除选定的人员吗?")){
				document.form.action = "${ROOT}/depriveRoleUser.do1";
				document.form.submit();
			}
		}
		...
		function viewRoleResource(){
			...
			window.location.href= "${ROOT}/viewRoleResource.do1?id=${role.id }";
		}
	//-->
	</script>
 </head>
...
<table class="list-box" cellspacing="0"  border="0" >
	...
	<tr class="list-color${idx.index%2==0?'1':'2' }">
		  <td align="left"><input name="uid" type="checkbox" value="${user.id }" class="checkboxInput"/>
		  ${idx.count}</td>
