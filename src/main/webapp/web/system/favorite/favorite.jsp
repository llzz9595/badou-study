...
function close2()
{
   ...
   window.close();
		...
		function add2(){
		    ...
		    var ret= window.showModalDialog('${ROOT}/createPersonFolder.do1?showType=dlg&parentId='+parentId, window, 'dialogHeight:220px;dialogWidth:350px;scroll=no;resizable=no;status=no');
			...
			//alert(ret);
			document.getElementById("ftree").src="${ROOT}/treePersonFolder.do1?jsFunc=list_folder&listType=select";
			  //alert( document.frames("ftree").location);
			 //  alert(document.getElementById("ftree"));
			 //document.frames("ftree").location.src="${ROOT}/treePersonFolder.do1?jsFunc=list_folder&listType=select";
                ...
                <td colspan="3" height="118px;" >
					...
					<table width="100%" border="0" cellpadding="1" cellspacing="1" class="define-right">
  					<tr>
						...
						<iframe id="ftree" src="${ROOT}/treePersonFolder.do1?jsFunc=list_folder&listType=select"  scrolling="auto"  frameborder="0" name="ftree" width="100%" height="100%"></iframe>
        				</div>
	  					</td>
  					</tr>
					</table>
