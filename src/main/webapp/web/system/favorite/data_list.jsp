	...
	<!--
	    function ev_showDate(parentId)
	    {
	      parent.document.frames("ftree").document.getElementById("listType").value="folder";
	       this.location.href="${ROOT}/listPersonFolder.do1?id="+parentId;
	    }
		/**
		*新增部门
		*/
		function ev_add(parentId){
			...
			var url = "${ROOT}/createPersonData.do1?parentId="+parentId;
		...
		*/
		...
		function ev_edit2(id) {
			var url = "${ROOT}/editPersonData.do1?showTime=first&id="+id;
		...
		*/
		function ev_delete(){
			...
			if(confirm("确定删除所选的记录吗？")){
				document.form1.action="${ROOT}/delPersonData.do1";
				document.form1.submit();
			}	
		}
		...
		function ev_move(parentId)
		{
		    ...
		    if(retvalue!=null&&retvalue!="")
		    {
		       ...
		       // alert("retvalue:"+retvalue);
		        document.form1.action="${ROOT}/movePersonData.do1?targetid="+retvalue;
				document.form1.submit();	
		    }
		     return ;
		}
		...
		{
			var url = "${ROOT}/swapPersonData.do1?currId="+currId+"&swapId="+swapId+"&parentId="+parentId;
			window.location = url;
		}
	//-->
	</script>
