	...
	<!--
		...
		*/
		function ev_add(orgId){
			var url = "${ROOT}/createPosition.do1?orgId="+orgId;
		...
		*/
		function ev_edit(id){
			var url = "${ROOT}/editPosition.do1?id="+id;
			//openWindow(url,750,450);
			pageShowForm(url);
		}
		...
		*/
		function ev_delete(){
			...
			if(confirm("确定要删除选定的岗位吗?")){
				...
				form.action = "${ROOT}/delPosition.do1?orgId=${orgId}";
				form.submit();
			}
		}
		...
		*/
		...
		function ev_query(){
			var form = document.getElementById("form");
			form.action = "${ROOT}/searchPosition.do1";
			form.submit();
		}
