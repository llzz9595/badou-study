	...
	<!--
		...
		*/
		function ev_add(parent_id){
			var url = "${ROOT}/createOrg.do1?parentId="+parent_id;
		...
		*/
		function ev_edit(id){
			var url = "${ROOT}/editOrg.do1?id="+id;
			//openFullScreenWin(url);
			pageShowForm(url);
		}
		...
		*/
		function ev_delete(){
			...
			if (!hadChecked) {
				...
				//effectDate = window.showModalDialog("${ROOT}/tip.do1?operateType=10",window,"dialogHeight:450px;dialogWidth:750px;status:no; help:no; scroll:no");
		...
		function ev_submit(){
			...
			//form1.action = "${ROOT}/delOrg.do1?parent_id=${parent_id}&effectDate="+effectDate;
			form1.action = "${ROOT}/delOrg.do1?parent_id=${parent_id}";
			form1.submit();
		}
		
		function ev_showHistory(orgID){
			window.showModalDialog("${ROOT}/listHistory.do1?id="+orgID,
		...
		*/
		function ev_moveOrg(){
				...
				//转到调动说明页面
				var retV = window.showModalDialog("${ROOT}/moveOrgsTargetOrgTip.do1?ids="+ids,window,"dialogHeight:500px;dialogWidth:600px;status:no; help:no; scroll:no");
				if(retV=='ok'){
					window.location = window.location;
					parent.frames['ftree'].location = parent.frames['ftree'].location;
				}
			}else{
				alert("请至少选择一个组织！");
			}
		...
		*/
		function ev_invadilOrg(){
			window.parent.location.href="${ROOT}/listInvadilOrg.do1";
		}
		//批量设置上级领导
		function ev_setLeader(){
			if (SelectUtil.checkedCount("id")>0) {
				...
				if(value!=""){
					form1.action = "${ROOT}/setLeaderByOrgs.do1";
					form1.submit();
				}
			}else{
				alert("请至少选择一个组织！");
...
<table class="list-box" cellspacing="0" border="0" cellpadding="0" >
	<tr align="left">
        ...
        <th align="center" width="10%" class="list-title" onclick="ev_showSet();">编码</th>
        <th align="center" width="*" class="list-title" onclick="ev_showSet();">名称</th>
        <th align="center" width="20%" class="list-title">创建时间</th>
    <%--    
...
<c:forEach var="o" items="${orgList}" varStatus="idx">
	<tr class="list-color${idx.index%2==0?'1':'2' }">
		...
		<td  class="list-text1" align="left"><a href="javascript:ev_edit('${o.id}');">${o.code}</a></td>
		<td  class="list-text1" align="left"><a href="javascript:ev_edit('${o.id}');">${o.name}</a></td>
		<td  class="list-text1"  align="left"><fmt:formatDate value="${o.createDate}" pattern="yyyy-MM-dd"/></td>
	</tr>
</c:forEach>
</table>
