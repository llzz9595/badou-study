...
<html>
  <head>
	...
	<!--
		...
		function ev_confirm(){
			if(CheckForm(form1) && confirm("你确定要调动组织?")){
...
<body class="LN_body">
<form name="form1" method="post" action="${ROOT}/moveOrg.do1" id="form1">	
...
<div class="LN_formTitle">组织调动</div>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="LN_infbox-border">
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="LN_tableBox">
        ...
        <font color="red">提醒：调动组织存在已发起考核表，在调整组织后，请在发起批次中对调动的组织进行"单独删除"，
        调整相关信息后再"单独发起", 以确保考核正常(点击下边考核表名称进入考核批次监控页面)。</font>
        </td>
        </tr></c:if>
      <tr>
        <td width="15%" class="LN_tableBoxtitle">需要调动组织</td>
      ...
      <tr>
        <td width="15%" class="LN_tableBoxtitle">调动组织已发起考核信息
        </td>
        <td width="90%" class="LN_tableBoxText">
       		<c:forEach var="o" items="${userList}">
