	...
	<!--
		...
		function ev_doMoveInto(reqID){
			...
			if(confirm("确定要调入选择的员工吗？")){
				var effectDate = window.showModalDialog("${ROOT}/tip.do1?reqID="+reqID+"&operateType=1",
										"","dialogHeight:450px;dialogWidth:750px;status:no; help:no; scroll:no");
				if(effectDate == undefined){
					alert("请选择调入生效期");
					return;
				}
				form.action = "${ROOT}/doMoveInto.do1?id="+reqID+"&effectDate="+effectDate;
				pageShield();
				form.submit();
			}
		}
		...
		*/
		function ev_moveIn(){
			...
			if(confirm("确定要调入选择的员工吗？")){
				var effectDate = window.showModalDialog("${ROOT}/tip.do1",
										"","dialogHeight:450px;dialogWidth:750px;status:no; help:no; scroll:no");
				if(effectDate == undefined){
					alert("请选择调入生效期");
					return;
				}						
				form.action = "${ROOT}/doMoveInto.do1?&orgId=${orgID}"+"&effectDate="+effectDate;
				pageShield();
				form.submit();
			}
		...
		*/
		function show_detail(id){
			var url = "${ROOT}/findMoveRequisition.do1?requisitionID="+id;
			openWindow(url,750,450);
		}
		...
		*/
		function ev_listMoveInto(){
			var url = "${ROOT}/listMoveRequisition.do1?type=0";
		...
		*/
		function ev_listMoveOut(){
			var url = "${ROOT}/listMoveRequisition.do1";
			window.location.href = url;
		}
		...
		*/
		function ev_cancelMove(){
			...
			var url = "${ROOT}/cancelMoveRequisition.do1";
	...
	<c:forEach var="requisition" items="${requisitionList}" varStatus="idx" >
		<tr class="list-color${idx.index%2==0?'1':'2' }">
			...
			<td>
				...
				<c:if test="${empty type }"><a id="hlinkDelete" href="${ROOT}/cancelMoveRequisition.do1?id=${requisition.id  }">取消申请</a></c:if>
