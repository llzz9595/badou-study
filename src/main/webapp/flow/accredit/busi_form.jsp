...
function ev_save()
{
	...
	if (method=="update")
	{
		formFrame.busiForm.action="${ROOT}/updateBusi.do1?busiID=<%=request.getParameter("busiID")%>";
	}
	if (method=="add")
	{
		formFrame.busiForm.action="${ROOT}/createBusi.do1";
	}
...
<table width="100%" height="100%"   cellpadding=0 border=0>
  ...
  <tr><td>
   <iframe scrolling="auto"   src="${ROOT}/viewBusi.do1?method=<%=request.getParameter("method")%>&busiID=<%=request.getParameter("busiID")%>"
