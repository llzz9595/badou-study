...
var aurl="${ROOT}/listWorkListByAccredit.do1?accreditId=<%=fa.getId()%>&busiType=<%=busiType%>&grantor=<%=grantor%>&acceptor=<%=acceptor%>";
...
<%}%>
	...
	function ev_exit()
	{
	   window.location.href="${ROOT}/listAccredit.do1";
	}
	...
	function ev_save(theParam)
	{
		...
		if (method=="update")
		{
			var url = "${ROOT}/checkAccredit.do1?aid=<%=fa.getId()%>&user="+user+"&obj="+obje+"&busyType="+busyType+"&start="+startDate+"&end="+endDate+"&nocache="+Math.random();
			LoadAjaxContentSyn(url,check);
			if(num>0)
			{
				num=0;return;
			}
			accreditForm.action="${ROOT}/updateAccredit.do1?theParam="+theParam;
		}
		if (method=="add")
		{
			var url = "${ROOT}/checkAccredit.do1?user="+user+"&obj="+obje+"&busyType="+busyType+"&start="+startDate+"&end="+endDate+"&nocache="+Math.random();
			LoadAjaxContentSyn(url,check);
			if(num>0)
			{
				num=0;return;
			}
			accreditForm.action="${ROOT}/createAccredit.do1?theParam="+theParam;
		}
	   	accreditForm.submit();
	}
	function check()
