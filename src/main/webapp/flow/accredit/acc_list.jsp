...
function changeAccreditType(){
	var url = "${ROOT}/listAccredits.do1?accreditType="+form1.accreditType.value;
	window.location.href = url;
}
function ev_add(){
	view_form("${ROOT}/viewAccredit.do1?");
}
function ev_del() {
	var number = 0;
	...
	if(confirm("确定删除所选的记录吗？")){
		document.form1.action="${ROOT}/delAccredit.do1?accreditType="+form1.accreditType.value;
		document.form1.submit();
	}	
}
function listAccreditUndo(accreditId){
	var url = "${ROOT}/listAccreditUndo.do1?accreditId="+accreditId;
	window.location = url;
}
...
<%--			<tr align="center" onmouseover='this.style.backgroundColor="#EFEFEF"'  style.cursor='default' onmouseout='this.style.backgroundColor="#ffffff"' bgColor=#ffffff>--%>
				...
				<td class="list-text1">
					<c:if test="${accreditType=='1' }">
					<a href="javascript: view_form('${ROOT}/viewAccredit.do1?accreditId=${a.id }');">
						${a.busiName }
					</a>
					</c:if>
					<c:if test="${accreditType!='1' }">
