...
<html:html>
		...
		<script>
			...
			function submitForm(form1,closeWindow1) {
				if(CheckForm(form1)&&checkJs()) {
					var url = "${ROOT}/isExistAccredit.do1?accreditId=${accredit.id}&busiId="+form1.busi.value+"&grantor="+form1.grantor.value;
			...
			function isExistAccredit(){
				...
				if(retVal == "false"){
					pageShield();
					form1.action = "${ROOT}/saveAccredit.do1?closeWindow="+closeWindow+"&id=${accredit.id}";
					form1.submit();
				}else{
					alert("该业务类型已经存在授权");
				}
	...
	<form id="form1" name="form1" method="post" action="${ROOT}/saveAccredit.do1"> 
