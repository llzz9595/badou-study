...
function ev_add()
{
   window.location.href="${ROOT}/system/common/info_form.jsp?method=add";
}
...
function ev_update()
{
  ...
  if (count ==1) {
  	 listFrame.infoForm.target="_parent";
	 listFrame.infoForm.action = "${ROOT}/system/common/info_form.jsp?method=update&infoID="+rowid;
	 listFrame.infoForm.submit();
  }
  else{
  	alert("请选择一条需要修改的记录！");
    return ;
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要删除记录？")) {
      	 listFrame.infoForm.target="_parent";
	     listFrame.infoForm.action = "/deleteInfo.do1";
         listFrame.infoForm.submit();
      }
    }else {
       alert("您还没选择记录!");
...
<table width="100%" height="100%"   cellpadding=0 border=0>
  ...
  <tr><td>
  <iframe scrolling="auto"   src="${ROOT}/listInfo.do1"  width=100% height=100% name="listFrame"></iframe>
