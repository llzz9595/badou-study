...
function ev_add()
{
   view_form("${ROOT}/viewAccredit.do1?method=add");
}
...
function ev_update()
{
  ...
  if (count ==1) {
	 view_form("${ROOT}/viewAccredit.do1?method=update&accreditID="+rowid);
  }
  else{
  	alert("请选择一条需要编辑的记录！");
    return ;
...
function ev_del()
{
    ...
    if (count >0)  {
      	if (window.confirm("您确认要删除记录？")) 
      	{
	     	...
	     	accreditForm.action = "${ROOT}/deleteAccredit.do1";
        	accreditForm.submit();
      	}
    }else {
       	alert("您还没选择记录!");
...
function ev_on()
{
    ...
    if (count >0)  {
      ...
      if (window.confirm("您确认要启动该授权？")) {
      	 accreditForm.target="_self";
	     accreditForm.action = "${ROOT}/onAccredit.do1";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
    ...
    if(str!="")
    {
    	...
    	return;
      	...
      	if (window.confirm("您确认要停止该授权？")) {
			accreditForm.target="_self";
		    accreditForm.action = "${ROOT}/offAccredit.do1";
	        accreditForm.submit();
      	}
    }else {
       alert("您还没选择记录!");
...
function ev_allundo()
{
	var lj="${ROOT}/listAllWorkLists.do1?uid=<%=loginedUser.getId()%>";
...
<%if(accreditList!=null)
 ...
 %>
	...
	<td align="left"><a href="javascript: view_form('${ROOT}/viewAccredit.do1?method=update&accreditID=<%=acc.getId() %>');">
			<%=busiName %>
		</a>
	</td>
