   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要删除记录？")) {
	     ...
	     accreditForm.action = "${ROOT}/deleteSelfAccredit.do1?action=listSelfReceiveAccredit.do1";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要启动该授权？")) {
      	 accreditForm.target="_self";
	     accreditForm.action = "${ROOT}/onSelfAccredit.do1?action=listSelfReceiveAccredit.do1";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要停止该授权？")) {
      	 accreditForm.target="_self";
	     accreditForm.action = "${ROOT}/offSelfAccredit.do1?action=listSelfReceiveAccredit.do1";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
