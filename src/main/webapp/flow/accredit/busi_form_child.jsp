...
function add_para(id){
    ...
    location.href = "${ROOT}/viewBusi.do1?method=update&busiID=<%=request.getParameter("busiID")%>&tab=2";   
    }
    }
    
   function editNodeExt(id){
	var returnValue = window.showModalDialog('${ROOT}/busiParamEditPage.do1?busiParamId='+id,'','dialogHeight:340px;dialogWidth:500px;status=off')
    if(returnValue=="success"){
    location.href = "${ROOT}/viewBusi.do1?method=update&busiID=<%=request.getParameter("busiID")%>&tab=2"; 
   }
}
...
if (confirm("您确认要删除记录？")) { 
document.busiForm.action = "${ROOT}/deleteBusiParamForType.do1?type=<%=request.getParameter("busiID")%>&id="+id;
...
function synchronize(){
	...
	var url ="${ROOT}/busiParamOfFlow.do1?busiCode=<logic:notEmpty name="commBusi"><bean:write name="commBusi" property="id"/></logic:notEmpty>&flgTempletSyc=1";
...
var url = "${ROOT}/listTempletForFlow.do1";
