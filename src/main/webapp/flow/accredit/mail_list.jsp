...
function ev_del(){
	...
	if(ids==""){
		...
		if(confirm("确定要删除所选中的邮件!")){
           form1.action = "${ROOT}/deleteMail.do1?";
           form1.submit();	
		} else {
           return;
        }
...
function view(id){
	var url = "${ROOT}/viewMail.do1?id="+id;
	var value = window.showModalDialog(url,"","dialogHeight:450px;dialogWidth:720px;status=off");
	if("refresh"==value){
		window.location.href="${ROOT}/listMail.do1";
	}
...
<FORM method="post" id="form1" name="form1" action="${ROOT}/listMail.do1">
