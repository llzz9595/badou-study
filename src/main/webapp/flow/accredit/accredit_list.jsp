   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要删除记录？")) {
      	 listFrame.accreditForm.target="_parent";
	     listFrame.accreditForm.action = "/deleteAccredit.do1";
         listFrame.accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要启动该授权？")) {
      	 listFrame.accreditForm.target="_parent";
	     listFrame.accreditForm.action = "${ROOT}/onAccredit.do1";
         listFrame.accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要停止该授权？")) {
      	 listFrame.accreditForm.target="_parent";
	     listFrame.accreditForm.action = "${ROOT}/offAccredit.do1";
         listFrame.accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
...
<table width="100%" height="100%"   cellpadding=0 border=0>
  ...
  <tr><td>
  <iframe scrolling="auto"   src="${ROOT}/listAccredit.do1"  width=100% height=100% name="listFrame"></iframe>
