...
function ev_add()
{
    view_form("${ROOT}/editInfo.do1?method=add");
}
...
function ev_update()
{
  ...
  if (count ==1) {
  	 view_form("${ROOT}/editInfo.do1?method=update&infoID="+rowid);
  }
  else{
  	alert("请选择一条需要修改的记录！");
    return ;
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要删除记录？")) {
	     ...
	     infoForm.action = "/deleteInfo.do1";
         infoForm.submit();
      }
    }else {
       alert("您还没选择记录!");
...
function ev_publish(){
	...
	var url = "${ROOT}/publishInfo.do1?ids=" + ids;
...
<table class=listtable bordercolor=#B3BBC8 bordercolordark=#ffffff align="center"
 ...
 <logic:iterate id="info" name="infoList" type="com.shengdai.kpi.common.valueobject.Info" scope="request" indexId="index" >
  <tr align="center" onmouseover='this.style.backgroundColor="#EFEFEF"'  style.cursor='default' onmouseout='this.style.backgroundColor="#ffffff"' bgColor=#ffffff>
	...
	<td align="left"><a href="javascript: view_form('${ROOT}/editInfo.do1?method=update&infoID=<bean:write name="info" property="id" />');">
