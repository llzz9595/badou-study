...
function transfer(){
	if(!selectRadio()){
	 ...
	 pageShield();
	 $_E("form1").action="${ROOT}/transferWorklistItem.do1?formNodeCode=${param.formNodeCode}";
	 $_E("form1").submit();
}
...
<table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0" class="define-box">
        <tr>
          <td class="dialog-titlebg" width="100%"><div class="dialog-title">
            ...
            <input type="button" class="button2" value="关闭" onclick="if(confirm('确定关闭？')) window.close();"/>
          </div></td>
        </tr>
        <tr>
