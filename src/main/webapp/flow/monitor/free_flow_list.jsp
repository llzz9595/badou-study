...
function viewFreeFlowMonitor(boId,userId,type,flowCode,flowType){
	...
	location.href="${ROOT}/viewFreeFlowMonitor.do1?batchId=<%=request.getParameter("batchId")%>&monitorId=<%=request.getParameter("monitorId")%>&boId="+boId+"&delegateId="+userId+"&delegateType="+type+"&flowCode="+flowCode+"&parentFlowCode=<%=request.getParameter("parentFlowCode")%>&type="+flowType;
}

function ev_back(){
	location.href="${ROOT}/listWorklistForRecBatch.do1?monitorId=<%=request.getParameter("monitorId")%>&id=<%=request.getParameter("batchId")%>";
}
