...
<script type="text/javascript">
	function ev_back(){
		if("${param.pageTag}"=="1"){
			window.location = "${ROOT}/watchMonitorStart.do1?id=${monitor.id}&orgId=${param.orgId}&busiId=${monitor.flow.commBusi.id}";
		}else{
			window.location = "${ROOT}/listWorklistForRecBatch.do1?monitorId=${monitor.id}&id=${batch.id}&orgId=${param.orgId}&busiId=${monitor.flow.commBusi.id}";
		}
	}
</script>
</head>
