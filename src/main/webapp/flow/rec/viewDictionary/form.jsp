...
function ev_save(flag){
	...
	form1.action = "${ROOT}/saveViewDictionary.do1?flag="+flag;
	form1.submit();
}
	...
	<form id="form1" name="form1" method="post" action="">
      ...
      <table class="formtext" border="0" cellpadding="0" cellspacing="1" style="width:98%">
        ...
        <tr>
          ...
          <td><input name="priority" value="${viewDictionary.priority }" showName="排序号" check="notBlank();isInt();" class="listinput" required="true"/></td>
        </tr>
		<tr>
...
<%--          	<input name="spreadUrl" value="${viewDictionary.spreadUrl }" class="listinput" />--%>
          </td>
        </tr>
        <tr>
          <td class="t">查阅URL</td>
...
<%--          	<input name="spreadUrl" value="${viewDictionary.spreadUrl }" class="listinput" />--%>
        ...
        <tr>
          <td class="t">已办URL</td>
          <td colspan="3">
          	<textarea name="doneUrl" cols="70" rows="2" showName="已办URL" check="isString('#',250)"  required="nullable">${viewDictionary.do1neUrl }</textarea>
<%--          	<input name="doneUrsl" value="${viewDictionary.do1neUrl }" class="listinput" />--%>
          </td>
        </tr>
        <tr>
          <td class="t">待办预览URL</td>
          <td colspan="3">
          	<textarea name="previewUrl" cols="70" rows="2" showName="待办预览URL" check="isString('#',250)"  required="nullable">${viewDictionary.previewUrl }</textarea>
          	
<%--          <input name="previewUrl" value="${viewDictionary.previewUrl }" class="listinput" />--%>
          </td>
        </tr>
        <tr>
          <td class="t">说明</td>
