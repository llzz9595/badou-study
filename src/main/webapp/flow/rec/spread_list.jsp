...
function toOpenSpread(wid,status){
	var getview = "${ROOT}/changeSpreadStatus.do1?wid="+wid+"&status="+status;
...
function openWin(boId,handlerId,nodeCode,batchId){
	var url="${ROOT}/openSpread.do1?boId="+boId+"&nodeCode="+nodeCode+"&batchId="+batchId;
...
function changeStatus(){
	var url = "${ROOT}/listSpread.do1?status="+form1.status.value;
	window.location = url;
}
...
function changeRead(){
	...
	if(j>0){
		form1.action="${ROOT}/changeSpread.do1";
		form1.submit();
	}else{
