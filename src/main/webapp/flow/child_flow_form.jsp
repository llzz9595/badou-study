...
parent.buttomFrame.Entrys.form1.action="${ROOT}/editFlow.do1?sCode="+parent.buttomFrame.tree.document.all.sCode.value;
...
parent.buttomFrame.Entrys.location.href = "${ROOT}/listNode.do1?flowCode=<%=request.getParameter("flowCode") %>";
}

function node_delete(){
if (confirm("您确认要删除记录？")) {
    parent.buttomFrame.Entrys.form1.action = "${ROOT}/deleteNode.do1?parentFlowCode=<%=request.getParameter("parentFlowCode")%>&flowCode=<%=request.getParameter("flowCode")%>&nodeCode=<%=request.getParameter("nodeCode")%>";
    parent.buttomFrame.Entrys.form1.submit();
    }else{
    return;
    }
...
<table width="100%" height="100%"   cellpadding=0 border=0>
  ...
  <tr>
  	<td><!--<iframe scrolling="auto"   src="${ROOT}/flowEditPage.do1?flowCode=<%=request.getParameter("flowCode") %>"
