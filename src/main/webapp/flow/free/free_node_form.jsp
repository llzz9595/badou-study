...
function editFreeNode(){
	if(checkJs()){
		form2.action = "${ROOT}/editFreeNode.do1";
		form2.submit();
	}
}
...
if(id!=""&&id!=undefined){
if(confirm("您确认要删除记录？")){
var nodeMinedeleteurl = "${ROOT}/deleteNodeMine.do1?id="+id+"&nodeCode=<%=request.getParameter("nodeCode")%>&tab=1&noCache="+Math.random();
...
form2.action = "${ROOT}/editNode.do1?flowCode=<%=request.getParameter("flowCode") %>&nodeCode=<%=request.getParameter("nodeCode") %>&remindIds="+ary;
form2.submit();
}

function updateFlwNodeForRoot(){
form2.action = "${ROOT}/editNodeForRoot.do1?flowCode=<%=request.getParameter("flowCode") %>&nodeCode=<%=request.getParameter("nodeCode") %>";
form2.submit();
}
...
function selectView(){
var url = "${ROOT}/selectDefaultViewPage.do1?nodeCode=<bean:write name="flwNode" property="nodeCode"/>";
