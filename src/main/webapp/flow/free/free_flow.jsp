...
var url = "${ROOT}/freeeNodeAddPage.do1?flwCode="+flowCode;
...
var url = "${ROOT}/deleteFreeNode.do1?nodeCode="+nodeCode;
 if (confirm("您确认要删除记录？")) { 
...
var url = "${ROOT}/sortFreeNode.do1?nodeCode="+nodeCode+"&type="+type+"&noCache="+Math.random();
...
%>
var flowCode = document.all.flwCode.value;
form1.action = "${ROOT}/startFreeFlow.do1?boClass=<%=request.getParameter("boClass")%>&boId=<%=request.getParameter("boId")%>&flwCode="+flowCode+"&parentWorklistItemId=<%=request.getParameter("parentWorklistItemId")%>&jsStr=window.opener.location.reload();";
form1.submit();
<%
}else{
%>
var flowCode = document.all.flwCode.value;
form1.action = "${ROOT}/submitFreeFlowNext.do1?thisNodeCode=<%=request.getParameter("thisNodeCode")%>&id=<%=request.getParameter("boInfo")%>&boId=<%=request.getParameter("boId")%>&flwCode="+flowCode+"&parentWorklistItemId=<%=request.getParameter("parentWorklistItemId")%>&jsStr=window.opener.location.reload();";
form1.submit();
<%
}
%>
...
if(hasWorklistItem){
%>
url = "${ROOT}/checkHandler.do1?id=<%=request.getParameter("boInfo")%>&flowCode="+document.all.flwCode.value+"&noCache="+Math.random();
<%
}else{
%>
url = "${ROOT}/checkHandler.do1?id=<%=request.getParameter("boInfo")%>&thisNodeCode=<%=request.getParameter("thisNodeCode")%>&noCache="+Math.random();
<%
}
%>
...
var url = "${ROOT}/listBackPage.do1?boId="+boid+"&fromNodeCode=<%=request.getParameter("thisNodeCode")%>";
