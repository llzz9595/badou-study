		...
		<script>
			...
			function loadFlow(select) {		
				...
				if(StringUtils.isNotEmpty(option.value)) {
					Util.show('bind');
					document.frames['flowFrame'].document.location.href = "${ROOT}/listFlowTreeForTemplate.do1?flowCode="+option.value;
				} else {
					Util.hide('bind');
			...
			function bindFlow() {
				...
				if(option && option.length>0){
					if(confirm(desc+'后已设定的考核关系会被删除，再次绑定需重新设置考核关系，你确定继续？')) {
						...
						var url = '${ROOT}/bindFlowTemplate.do1?templateId=${param.templateId}&flowId='+option+'&templateType=${param.templateType}';
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="LN_infbox-border">
  ...
  <tr>
  	<td class="LN_tableBoxText">
  	<iframe id="flowFrame" src="${ROOT}/listFlowTreeForTemplate.do1?flowCode=${flowCode}" width="100%" height="300px" frameborder="0"></iframe>
  	</td>
  </tr>
  </table>
  </div>
