...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '显示从{from}到{to}, 总 {total} 条. 每页显示 {pagesize} 条',
        ...
        */
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                var containerLeft = offset.left, containerTop = height > 30 ? offset.top + height/2 - 15 : offset.top;
                var container = $("<div class='l-grid-editor'></div>").appendTo('body');
                if ($.browser.mozilla)
                    container.css({ left: containerLeft, top: containerTop }).show();
                else
                    container.css({ left: containerLeft + 1, top: containerTop + 1 }).show();
