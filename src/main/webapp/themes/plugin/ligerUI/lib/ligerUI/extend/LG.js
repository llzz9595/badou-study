(function ($)
{
    ...
    //创建过滤规则数据(查询表单)
	LG.bulidFilterParams = function(form){
		if (!form) return null;
		var params = new Map();
    ...
    //附加表单搜索按钮：搜索、高级搜索
    LG.appendSearchButtons = function (form, grid, hasHSearch)
    {
        if (!form) return;
        form = $(form);
        ...
        if (btn1Container)
        {
            LG.createButton({
                ...
                click: function ()
                {
                    ...
                    //    grid.set('parms', {});
                    //}
            		//重新搜索事件,将各参数直接以JSON格式附加到请求参数params后grid.set('parms', { code:'123',name:'张三'});
            		var json = LG.bulidFilterParams(form);
            		grid.set('parms', json);
					grid.loadData();
                }
            });
    	...
    	$.ajax({
		            ...
		            complete: function (){
   					...
   					success: function(data){
		                ...
		                //加载子单, update xiangsf 2013-1-24暂不可用 
		                if(options.detailForm){
		                	var detailForm = options.detailForm;
		                	var childJsonName = options.childName || "children";
		                	//alert(JSON.stringify(data[childJsonName]));
		                	//detailForm.addRows(data[childJsonName]);
		                }
		            	if (options.callback)
    ...
    //创建多个按钮
	LG.createToolbarButton = function(grid, options){
		...
		var items = [];
                for (var i = 0; i < options.length; i++){
                    ...
                    items[items.length] = {
                        click: o.clickEvent,
                        text: o.name,
                        img: baseURL + "/themes/default/icons" + o.icon,
                        id: o.id
                    };
    ...
    LG.loadToolbar = function (grid, toolbarBtnItemClick)
    {
        ...
        LG.ajax({
            loading: '正在加载工具条中...',
            type: 'AjaxSystem',
            method: baseURL+'/loadFunctionButton.do1',
            data: { menuId: MenuNo },
            success: function (data)
            {
                if (!grid.toolbarManager) return;
    ...
    //设置grid的双击事件(带权限控制)
    LG.setGridDoubleClick = function (grid, btnItemClick, btnId)
    {
	...
	LG.serialIdParam = function (datas){
