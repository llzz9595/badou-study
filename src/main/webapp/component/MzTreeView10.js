...
MzTreeView.prototype.expand   = function(id, sureExpand)
{
  ...
  if (area)
  {
    ...
    //在此将记忆功能集成，不管状态是怎么样的，只要点击了+号就增加入cookie，点击了-号就从cookie删除
    var srcId = node.sourceIndex.split("_")[1];
    if(Bool){    	
    	this.setCookie(srcId, 1);
    } else {
    	this.setCookie(srcId, 0);
    }
...
///////////////////////////////////////////////////////////////////////////////
/*
* 在cookies中添加和删除记忆节点，被树主件节点的展开和关闭所调用。
* value=1说明添加记忆节点，value=0删除记忆节点，也就是说点击+号展开是1，点击-号收起是0
*/
MzTreeView.prototype.setCookie = function (key, value) {
	if(this.getCookie("_menuInfo")==null){
		document.cookie="_menuInfo=;";
	}
	if(value=="1"){
		this.setMenuCookie(key, value);
	}else if(value=="0"){
		this.delMenuCookie(key);
	}
}
 ...
 * 增加记忆节点
 */
MzTreeView.prototype.setMenuCookie = function (key, value) {
	var tmpMenuCookie=this.getCookie("_menuInfo");
	...
	if(tmpMenuCookie.indexOf(key)==-1){
		if(tmpMenuCookie!=""){
			tmpMenuCookie+="$"+key;
		}else{
			tmpMenuCookie=key;
	...
	document.cookie="_menuInfo=" + tmpMenuCookie;			//重新设置cookies
		//获取当前时间
	var date_cookie=new Date();
	var expireDays=365;			//默认设置保存一年
	//将date设置为10天以后的时间
	date_cookie.setTime(date_cookie.getTime()+expireDays*24*3600*1000);
	//将_menuInfo的cookie设置为10天后过期
	document.cookie="_menuInfo=" + tmpMenuCookie+"; expires="+date_cookie.toGMTString();			//重新设置cookies
	this.cookies = document.cookie;
 ...
 * 删除记忆节点
 */
MzTreeView.prototype.delMenuCookie = function (key) {
	var tmpMenuCookie=this.getCookie("_menuInfo");
	if(tmpMenuCookie==null){
		document.cookie="_menuInfo=;";
		this.cookies = ' ' + document.cookie;
	...
	if (tmpMenuCookie!=null) {
		...
		var tmpcookieEnd = tmpMenuCookie.substr(end,tmpMenuCookie.length);
 ...
 * 返回记忆节点数组
 */
MzTreeView.prototype.getMenuCookieNodes = function (key) {
	var treeNodes=this.getCookie("_menuInfo");
	if(treeNodes!=null){
		return treeNodes.split("$");
	}else{
		return null;
	}
}
...
MzTreeView.prototype.getCookie = function (key) {
		if (this.cookies) {
 ...
 * 将曾经展开过的节点展开
 */
MzTreeView.prototype.showHistory = function(){
	var keys = this.getMenuCookieNodes();
	if(keys){
