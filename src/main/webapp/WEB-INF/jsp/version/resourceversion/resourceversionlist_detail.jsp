...
.l-button1-over,.l-button1:hover {
	...
	<body style="padding-bottom: 31px;">
		...
		<div id="detailCost" style="">
			...
			<form id="formdetail" name="formdetail" method="post" class="l-form">
					...
					<tr height="30">
						...
						<td>
							<input type="text" inputwidth="220" name="SDdetail_versionNum"
								id="SDdetail_versionNum" value="${resourceVersionForm.versionNum}"
						...
						<td>
							<input type="text" inputwidth="220" ltype="text"
								...
								value="${resourceVersionForm.versionName}"
					...
					<tr height="30">
						...
						<td colspan="3">
							<input type="text" inputwidth="632" ltype="text"
								...
								value="${resourceVersionForm.versionDescription}"
						...
						<td colspan="4">
							<div id="upload">
								<input name="attach" id="attach" type="file" class="field"
										...
										inputwidth="635" value="" size="94"/>
							</div>
							<div id="download" style="display: none;">
								<span style="color: #53CBF1; size: 12px;">${resourceVersionForm.attachName}</span>
								<a href="${baseURL }/version/resourceversion/resourceversionsave/downloadFile.do?resourceVersionId=${resourceVersionForm.id }">下载</a>
									<a href="#"
										onclick="deleteAttach('${resourceVersionForm.id }');return false;">删除</a>
								</div>
						</td>
				...
				<input type="hidden" name="SDdetail_apkId" id="SDdetail_apkId" value="${versionApkId}"/>
				<input type="hidden" name="SDdetail_id" id="SDdetail_id" value="${resourceVersionForm.id}"/>
		...
		function initFile() {
		<c:if test="${empty resourceVersionForm || empty resourceVersionForm.attachName }">
			$("#upload").show();
			$("#upload1").show();
			$("#download").hide();
		</c:if>
		<c:if test="${not empty resourceVersionForm && not empty resourceVersionForm.attachName }">
			$("#upload").hide();
			$("#upload1").hide();
			$("#download").show();
		</c:if>
	...
	initFile();
		...
		function FF_Save() {
			...
			formdetail.attr("action","${baseURL}/version/resourceversion/resourceversionsave/resourceVersionSave.do");
			LG.submitForm(formdetail, function(data) {
				
				if (data && data.hasOk) {
						
					alert('操作成功!');
					win.LG.closeAndReloadParent("resourceVersionEdit", "resourceVersionList");
					//win.LG.closeCurrentTab("resourceVersionEdit");
		...
		//删除附件
		 function deleteAttach(resourceVersionId) {
	    	$.ajax({
				type:"post",
				dataType:"json",
				url:"${baseURL }/version/resourceversion/resourceversionsave/deleteFile.do?resourceVersionId="+resourceVersionId,
				async: false,
