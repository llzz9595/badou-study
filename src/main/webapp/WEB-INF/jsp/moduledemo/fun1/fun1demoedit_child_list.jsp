...
<script type="text/javascript">
 	var childrenDetailsGrid = $("#childrenlist").ligerGrid({ 
		columns: [
		    ...
		    {display:"创建时间",name:"createTime",width:180,type:"date",align:"center",isSort : true},
		    {display:"创建人",name:"creatorName",width:100,type:"text",align:"center",isSort : true},
		    {display:"字段1",name:"fieldName1",width:80,type:"text",align:"center",isSort : false},
		    {display:"字段2",name:"fieldName2",width:80,type:"int",align:"center",isSort : false}
		],
		usePager : false, title:'示例1子单列表',showTitle:true,
        url : '${baseURL}/moduledemo/fun1/children/fun1demochildlist/listJSON.do?fun1DemoId=${id}',
 	...
 	//工具条事件
  	...
  	function FF_ModifyClickEvent(btnItem){
		...
		FF_ShowDialog({
			...
			fieldName2 : row.fieldName2, priority : row.priority
		}, false);
  	}
		...
		var selectedRow = childrenDetailsGrid.getCheckedRows();
     	...
     	jQuery.ligerDialog.confirm('您确定删除吗?', function (confirm) {
           	if (confirm){
            	$.ajax({
   					type: "GET",
   					url: "${baseURL}/moduledemo/fun1/children/fun1demochilddelete/delete.do",
   					dataType : "json",
